...
func postStream(room string) {
	//Encode the data
	var buf []byte
	responseBody := bytes.NewBuffer(buf)
	//Leverage Go's HTTP Post function to make request
	resp, err := http.Post(BACKEND_ADDR+"stream/add/"+room, "application/json", responseBody)
	//Handle Error
	if err != nil {
		return
...
func (s *Server) handleConn(conn *core.Conn) error {
	...
	if connServer.IsPublisher() {
		...
		if pushlist, ret := configure.GetStaticPushUrlList(appname); ret && (pushlist != nil) {
			log.Error("GetStaticPushUrlList: %v\n", pushlist)
		}
		reader := NewVirReader(connServer)
		s.handler.HandleReader(reader)
		room := reader.Info().Key
		if s.getter != nil {
		...
		if configure.Config.GetBool("flv_archive") {
			...
			fmt.Println(flvWriter)
			s.handler.HandleWriter(flvWriter.GetWriter(reader.Info()))
		}
		//HERE TO CREATE DB ENTRY
		postStream(room[5:len(room)])
		fmt.Println("Some guy is streaming wtf!!!")
	} else {
