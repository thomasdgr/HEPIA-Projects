/*******************************************************************************
           ...
           | _ _`\|              |
    .-.   `| O O |               |  Auteur : Thomas Dagier w/ Florent Gluck
    | |   (- /_  -)              |
    | |    |     |               |  Descripition : Fichier entièrement
 __/  |    | \__/|               |    repris sur la base de Florent Gluck
(___) \.  _.\__. `\___           |    ajout d'une fonction pour simplifier la
(___)  )\/  \_____/  ~\.         |    lecture et l'éxécution du paramètre
(___) . \   `         `\         |    graphique
 (__)-    ,/        (   |        |
      `--~|         |   |        |  Version : 1.0
          |         |   |		     |	Date :	29/11/2019
...
void render_image(int m, int n, struct gfx_context_t** context, matrix mat){
  if(*context==NULL) {
      *context = gfx_create("Gustavo.pgm", n, m);
      global_m = m;
      global_n = n;
  }
