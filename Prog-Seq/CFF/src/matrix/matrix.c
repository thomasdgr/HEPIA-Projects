/*******************************************************************************
 ...
 __/  |    | \__/|               |    utilisées pour appliquer des modifications
(___) \.  _.\__. `\___           |    sur des matrices
(___)  )\/  \_____/  ~\.         |
(___) . \   `         `\         |
 (__)-    ,/        (   |        |
      `--~|         |   |        |  Version : 1.0
...
matrix matrix_create(int m, int n){
  ...
  if(m <= 0 || n<= 0){
    ...
    mat.data[0]= malloc(n * m * sizeof(int));
...
int matrix_fill(matrix mat, int n){
  if(matrix_is_valide(mat)){
    for (int i = 0; i < mat.m; i++) {
      for (int j = 0 ; j < mat.n; j++) {
        // on ne remplie pas la diagonale
         if(i != j){
          mat.data[i][j] = n;
         } else {
          mat.data[i][j] = 0;
...
int matrix_add_scalar_in_place(matrix mat, int n){
  if(matrix_is_valide(mat)){
    for(int i = 0; i < mat.m; i++){
      for(int j = 0; j < mat.n; j++){
...
int matrix_mult_scalar_in_place(matrix mat, int n){
  if(matrix_is_valide(mat)){
    for(int i = 0; i < mat.m; i++){
      for(int j = 0; j < mat.n; j++){
        ...
        for (int k = 0; k < kernel.m; k++){
          for (int l = 0; l < kernel.n; l++){
            ...
            if((i-Kx+k) < mat1.m && (i-Kx+k) >= 0 && (j-Ky+l) < mat1.n && (j-Ky+l) >= 0){
              somme += (int)kernel.data[k][l] * (int)mat1.data[(i-Kx+k)][(j-Ky+l)];
            }
          }
        }
        mat->data[i][j] = somme;
