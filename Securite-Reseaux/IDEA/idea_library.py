"""*****************************************************************************
 ...
 __/  |    | \__/|         |    le chiffrement et le déchiffrement du message.
(___) \.  _.\__. `\___     |    Cela permet la mise en place de l'algorithme IDEA
(___)  )\/  \_____/  ~\.   |
(___) . \   `         `\   |
 (__)-    ,/        (   |  |
      `--~|         |   |  |  Version : 1.0
...
def decryption_idea(key32bits, cypher16bits):
	...
	for  round in range (0,5):
	    ...
	    key = calculate_decryption_key(keyArray, round, 0, 0)            #step1
	    result1 = mult_nibbles(textArray[0], key, 17)
	    key =  calculate_decryption_key(keyArray, round, 1, 1)           #step2
	    result2 = add_nibbles(textArray[1], key, 16)
	    key = calculate_decryption_key(keyArray, round, 2, 1)            #step3
	    result3 = add_nibbles(textArray[2], key, 16)
	    key = calculate_decryption_key(keyArray, round, 3, 0)            #step4
...
def generate_pseudo_random_key32bits(bitSize):
	...
	numberString = convert_int_to_string_binary(number)
	while len(numberString) < 32:
