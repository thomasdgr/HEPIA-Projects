...
int matrix_mult_in_place(matrix *mat1, matrix mat2){
  //Multiplie les données de mat1 par celles de mat1.
  //La matrice mat1 est modifiée.

  int nombreLigneMatrice1= mat1->m;
  int nombreColonneMatrice1= mat1->n;
  int nombreLigneMatrice2= mat2.m;
  int nombreColonneMatrice2= mat2.n;
  double tableauResultat[nombreLigneMatrice1][nombreColonneMatrice2];
  //matrix resultatOperation= matrix_create()
  if(nombreColonneMatrice1==nombreLigneMatrice2 && matrix_is_valide(*mat1) && matrix_is_valide(mat2))
  {
    //Calcul de la multiplication pour chaque valeur.
    for(int i=0;i<nombreLigneMatrice1;i++)
    {
      for(int j=0;j<nombreColonneMatrice2;j++)
      {
        tableauResultat[i][j]=0;
        for(int k=0;k<nombreColonneMatrice1;k++)
        {
          tableauResultat[i][j]+=mat1->data[i][k]*mat2.data[k][j];

        }
      }
    }

    //On redimensionne la matrice 1 car dans une multiplication matricielle la taille change.
    matrix_resize(mat1,nombreLigneMatrice1,nombreColonneMatrice2);
    //On recopie les valeurs du tableau résultat dans la matrice initiale
    for(int i=0;i<nombreLigneMatrice1;i++)
    {
      for(int j=0;j<nombreColonneMatrice2;j++)
      {
        mat1->data[i][j]= tableauResultat[i][j];
      }
    }

    return 1;
  }
    //printf("Erreur: Matrices de mauvaises tailles. Impossible de multiplier\n");
  return 0;
}
...
matrix matrix_create_identity_matrix(int m, int n){
  matrix mat = matrix_create(m,n);
  for(int i = 0; i < mat.m; i++){
    for (int j = 0; j < mat.n; j++){
      if(i == j){
