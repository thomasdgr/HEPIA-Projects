...
#Ouverture du fichier pas en binaire pour ecrire le dictionnaire en UTF-8
output_text = open(output_bin, "w+",encoding='utf8')
output_content = ""
...
for i in dichuffman:
        huffman_output += i + ": " + str(dichuffman[i]) + " \n"
...
for key in list(dichuffman.keys()) :
    output_content += key + ":" + dichuffman[key] + "\n"
    output_text.write(output_content)
    output_content = ""
...
#On ferme le fichier pour le reouvrire en binaire
output_text.close()
#Pour ne pas écraser les données, on fait ab au lieu de wb
output_text = open(output_bin, "ab")
...
#On voit le nombre de bits "sans-octet"
extra = len(output_content) % 8
#Le nombre de '0' à rajouter pour le padding
extra = 8 - extra

#On écrit dans le premier octet du message
output_bytes.append(extra)
